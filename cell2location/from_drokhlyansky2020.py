...
mod.train(max_epochs=100, batch_size=32768)
...
rdata = mod.export_posterior(
    rdata,
    sample_kwargs={"num_samples": 5000, "batch_size": 10000},
)
print(rdata)
...
# ---------------------------------------------------------------------------
# Save reference model and annotated rdata before freeing memory
# ---------------------------------------------------------------------------

regression_model_dir = os.path.join(OUTDIR, "regression_model")
mod.save(regression_model_dir, overwrite=True)
print(f"Saved RegressionModel -> {regression_model_dir}/")

rdata_path = os.path.join(OUTDIR, "rdata_with_posterior.h5ad")
rdata.write_h5ad(rdata_path)
print(f"Saved reference AnnData with posterior -> {rdata_path}")
...
shutil.copyfile(H5AD_QRY, tmp_qry)  # copyfile does not copy permissions
os.chmod(tmp_qry, 0o644)              # ensure rw so h5py can open r+
try:
...
# ---------------------------------------------------------------------------
# Export Cell2location posterior — this populates qdata.obsm with per-spot
# cell type abundance estimates (means, 5th/50th percentiles, stds)
# ---------------------------------------------------------------------------

print("Exporting Cell2location posterior (num_samples=1000)...")
qdata = mod.export_posterior(
    qdata,
    sample_kwargs={"num_samples": 10000, "batch_size": 2500},
)
print(qdata)

# QC plot for Cell2location
save_plot(
    os.path.join(OUTDIR, "c2l_QC.png"),
    plot_fn=mod.plot_QC,
)

# Save the trained Cell2location model
c2l_model_dir = os.path.join(OUTDIR, "c2l_model")
mod.save(c2l_model_dir, overwrite=True)
print(f"Saved Cell2location model -> {c2l_model_dir}/")

# Save annotated spatial AnnData — the primary output
qdata_path = os.path.join(OUTDIR, "qdata_with_abundances.h5ad")
qdata.write_h5ad(qdata_path)
print(f"Saved spatial AnnData with cell abundances -> {qdata_path}")

# Summarise the obsm keys added by export_posterior
print("\nobsm keys in qdata after export_posterior:")
for k in qdata.obsm.keys():
    print(f"  {k}: shape {qdata.obsm[k].shape}")
